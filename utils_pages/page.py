...
def users(request):
    ...
    page_obj = Pagination(current_page,total_item_count,'/users.html')  # 需要写入URL，在哪个URL下生成就写哪个URL

    user_list = models.UserInfo.objects.all()[page_obj.start:page_obj.end]

    return render(request,'users.html',{'user_list':user_list,'page_html':page_obj.page_html()})  # 返回的时候也需要URL

在前端html页面也要引入{{page_html}}
"""
